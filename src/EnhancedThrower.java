 ...
 *
 */
public class EnhancedThrower implements ValidThrows{
	...
	private void selectBestThrowInArray(){
		if (randomIndex > charArrayOfThrows[0].length){
			for (int i = 0; i < charArrayOfThrows.length; i++){
				if (charArrayOfRecordedThrows[mostThrows] == charArrayOfThrows[i][0]){
					...
					bestThrow = charArrayOfThrows[i][randomIndex];
			...
			bestThrow = charArrayOfThrows[0][randomIndex];
			// Step Five - new AI feature
			bestThrow = makeAiGreatAgain();
		}
	}
	...
	public char computeBestThrow(){
		...
		if (((arrayOfRecordedThrows[mostThrows] / totalThrows) * 100) >= 40){
			randomIndex = random.nextInt(charArrayOfThrows[0].length + 4);
			selectBestThrowInArray();
		}
		if (((arrayOfRecordedThrows[mostThrows] / totalThrows) * 100) >= 60){
			randomIndex = random.nextInt(charArrayOfThrows[0].length + 8);
			selectBestThrowInArray();
		}
		if (((arrayOfRecordedThrows[mostThrows] / totalThrows) * 100) > 80){
			randomIndex = random.nextInt(charArrayOfThrows[0].length + 20);
			selectBestThrowInArray();
		}
		return bestThrow;
	}
	...
	private char makeAiGreatAgain(){
		...
		for (int i = 0; i < charArrayOfThrows.length; i++){
			if (bestThrow == charArrayOfThrows[i][0]){
				if (leastCommonThrow == charArrayOfThrows[i][1] || leastCommonThrow == charArrayOfThrows[i][2]){
